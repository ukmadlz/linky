...
export function SchedulingPanel({ block, pageId, onUpdate }: SchedulingPanelProps) {
  const [start, setStart] = useState(
    toDatetimeLocal(block.scheduledStart ? new Date(block.scheduledStart) : null)
  );
    ...
    <details className="rounded-lg border border-slate-200 bg-white">
      ...
      <div className="flex flex-col gap-3 border-t border-slate-100 px-3 pb-3 pt-3">
        <p className="text-xs text-slate-500">
          Optionally set a window when this block is visible. Leave blank to always show.
        </p>

        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-slate-600">Show from</label>
            <input
              type="datetime-local"
              value={start}
