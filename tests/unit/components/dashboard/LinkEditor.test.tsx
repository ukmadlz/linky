...
describe("LinkEditor", () => {
	...
	it("shows mode selectors after enabling verification toggle", async () => {
		...
		expect(
			screen.getByRole("radio", { name: /age gate/i }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("radio", { name: /content warning/i }),
		).toBeInTheDocument();
	});
	...
	it("shows age gate description when age mode selected", async () => {
		...
		expect(screen.getByText(/age verification screen/i)).toBeInTheDocument();
	});
