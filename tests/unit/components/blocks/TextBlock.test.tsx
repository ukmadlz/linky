...
describe("TextBlock", () => {
  it("renders an <h2> for variant: heading", () => {
    const block = makeBlock({ content: "Hello", variant: "heading", align: "center" });
  ...
  it("renders a <p> for variant: paragraph", () => {
    const block = makeBlock({ content: "A paragraph", variant: "paragraph", align: "left" });
  ...
  it("applies text-left class for align: left", () => {
    const block = makeBlock({ content: "Text", variant: "paragraph", align: "left" });
    const { container } = render(<TextBlock block={block} />);
    expect(container.firstChild).toHaveClass("text-left");
  });

  it("applies text-center class for align: center", () => {
    const block = makeBlock({ content: "Text", variant: "paragraph", align: "center" });
    const { container } = render(<TextBlock block={block} />);
    expect(container.firstChild).toHaveClass("text-center");
  });

  it("applies text-right class for align: right", () => {
    const block = makeBlock({ content: "Text", variant: "paragraph", align: "right" });
    const { container } = render(<TextBlock block={block} />);
