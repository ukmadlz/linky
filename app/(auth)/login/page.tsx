import Link from "next/link";
import { Metadata } from "next";

export const metadata: Metadata = {
  title: "Sign in",
};

export default function LoginPage({
  searchParams,
}: {
  searchParams: { error?: string; next?: string };
}) {
  const errorMessages: Record<string, string> = {
    no_code: "Authentication failed. Please try again.",
    auth_failed: "Something went wrong. Please try again.",
  };

  const errorMessage = searchParams.error
    ? errorMessages[searchParams.error] ?? "An error occurred."
    : null;

  return (
    <div className="bg-white rounded-2xl shadow-xl border border-border p-8 md:p-10">
      {/* Logo / Brand */}
      <div className="text-center mb-8">
        <Link href="/" className="inline-block">
          <span className="font-serif text-3xl font-bold text-foreground tracking-tight">
            Linky
          </span>
        </Link>
        <h1 className="font-serif text-2xl font-semibold text-foreground mt-4">
          Welcome back
        </h1>
        <p className="text-muted-foreground mt-2 text-sm">
          Sign in to manage your link-in-bio page
        </p>
      </div>

      {/* Error message */}
      {errorMessage && (
        <div className="mb-6 p-3 bg-destructive/10 border border-destructive/20 rounded-lg text-sm text-destructive text-center">
          {errorMessage}
        </div>
      )}

      {/* Google OAuth button */}
      <a
        href="/api/auth/oauth"
        className="flex items-center justify-center gap-3 w-full h-12 px-6 bg-primary text-primary-foreground rounded-lg font-medium text-sm hover:scale-[1.02] hover:brightness-110 transition-all duration-200 shadow-sm"
      >
        {/* Google icon */}
        <svg
          width="18"
          height="18"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
          <path
            fill="currentColor"
            fillOpacity="0.9"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="currentColor"
            fillOpacity="0.7"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="currentColor"
            fillOpacity="0.5"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="white"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        Sign in with Google
      </a>

      <p className="text-center text-xs text-muted-foreground mt-6">
        By signing in, you agree to our{" "}
        <Link href="/terms" className="underline hover:text-foreground">
          Terms of Service
        </Link>{" "}
        and{" "}
        <Link href="/privacy" className="underline hover:text-foreground">
          Privacy Policy
        </Link>
        .
      </p>
    </div>
  );
}
