...
const IFRAME_PATTERNS: IframePattern[] = [
	...
	{
		match: /vimeo\.com\/(\d+)/,
		transform: (_url, match) => `https://player.vimeo.com/video/${match[1]}`,
		aspectRatio: "16/9",
		providerName: "Vimeo",
	},
	// SoundCloud
	...
	// Stripe Payment Links: buy.stripe.com/XXXX
	{
		...
		transform: (_url, match) => `https://buy.stripe.com/${match[1]}#embedded`,
		aspectRatio: "3/4",
		providerName: "Stripe",
	},
	...
	{
		match:
			/music\.apple\.com\/([a-z]{2})\/(?:album|playlist|artist)\/[^/]+\/([a-z0-9.-]+)/,
		transform: (url) => {
			const embedUrl = url
	...
	// TikTok: tiktok.com/@user/video/ID or vm.tiktok.com/shortcode
	{
		...
		transform: (_url, match) => `https://www.tiktok.com/embed/v2/${match[1]}`,
		aspectRatio: "9/16",
		providerName: "TikTok",
	},
